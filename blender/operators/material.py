...
class SSSekaiGenericMaterialSetModeOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for obj in active_object + [active_object.children_recursive]:
            obj: bpy.types.Object
            if obj.type == "MESH":
                for mat in obj.data.materials:
                    for node in mat.node_tree.nodes:
                        if node.name.startswith("SSSekai"):
                    ...
                    def set_nodegroup(node_group_name: str):
                        ...
                        mat.node_tree.links.new(
                            node_group.outputs[0], output_node.inputs[0]
                        )

                    match wm.sssekai_generic_material_import_mode:
                        case "UNITY_PBR_STANDARD":
                            node_group = set_nodegroup("SSSekaiUnityPBRStandard")
                        case "BASIC":
                            node_group = set_nodegroup("SSSekaiBasic")
                        case "BASIC_TOON":
                            node_group = set_nodegroup("SSSekaiBasicToon")
                        case "EMISSIVE":
                            node_group = set_nodegroup("SSSekaiEmissive")
        return {"FINISHED"}
