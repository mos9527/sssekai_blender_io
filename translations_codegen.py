        ...
        for pa in ast.walk(src):
            for node in ast.iter_child_nodes(pa):                
                if isinstance(node, ast.Call) and getattr(node.func,'id', None) == 'T':
                    yield f'scripts/addons/sssekai_blender_io/{filename}:{pa.lineno}', node.args[0].value
translations = defaultdict(lambda: defaultdict(list))

for (msgctxt, msgid), (sources, gen_comments), (lang, translation, (is_fuzzy, comments)) in translations_tuple:
...
for source in SOURCES:
    for sources, text in parse_ast(source):
        for lang in LANGS:
            ...
            has_tl = any((elem[-1] for elem in translations[text][lang])) if has_lang else False
...
for msgid, translations_dict in translations.items():
    for lang, translations_list in translations_dict.items():
        for msgctxt, sources, translation in translations_list:
            translations_tuple.append(((msgctxt, msgid), (sources, ""), (lang, translation, (False, ()))))
translations_tuple = sorted(translations_tuple, key=lambda x: x[0][1]) # key=msgid
translations_tuple = sorted(translations_tuple, key=lambda x: bool(x[2][1])) # key=translation
# Sorted by msgid, and then translation status
with open('translations.py', 'w', encoding='utf-8') as f:
   from pprint import pformat
   f.write('''# This file is auto-generated by generate_translations.py
